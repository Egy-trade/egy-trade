...
class TestOfferExpiry(SavepointCase):
    ...
    def _order(self, **values):
        ...
        if (
            "offer_expiry_days" in defaults
            and defaults["offer_expiry_days"] != self.company.quotation_expiry_days_default
        ):
    ...
    def test_order_column_default_is_schema_safe(self):
        ...
        self.assertEqual(field.default(self.env["sale.order"]), 14)
    ...
    def test_create_uses_company_default_and_timezone_business_date(self):
        ...
        expected_offer_date = fields.Date.context_today(
            order.with_context(tz="Africa/Cairo")
        )
    ...
    def test_draft_save_does_not_refresh_offer_date_or_date_order(self):
        ...
        self.assertEqual(
            order.validity_date,
            old_offer_date + timedelta(days=1),
        )

    def test_per_quotation_days_recomputes_expiration_on_save(self):
        order = self._order(offer_expiry_days=5)
        order.write({"offer_expiry_days": 21, "finance_approval_reason": "Expiry test override"})
