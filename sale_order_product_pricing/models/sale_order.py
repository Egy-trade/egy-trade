...
class SaleOrder(models.Model):
    ...
    product_pricing_ids = fields.One2many('sale.order.line', 'order_id', store=False, compute='change_order_line',
                                          readonly=False,precompute=True)
    currency_estimate_id = fields.Many2one('res.currency', string='Currency', default=lambda self: self.env.company.currency_id)
    currency_estimate_id_symbol = fields.Char(related='currency_estimate_id.symbol')
    currency_rate_estimate = fields.Float(compute='_compute_currency_rate',digits=(12, 6), store=True, precompute=True)
