...
| R07 | Manager, then Lighting Designer | Before release, inspect the drawing file itself and confirm it contains no prices, estimates, margins, vendors, client organization, or client-contact details. Release it, then open Technical Quotations and Technical Drawings as the assigned Lighting Designer. | Only assigned project scope, reviewed/released drawings, products, descriptions, quantities, and revision status are visible. No quotation record, client organization/contact, owner team/activity, price, total, estimate, margin, or vendor access exists. File-content review is a mandatory human gate; the module does not scan uploaded document contents. |
