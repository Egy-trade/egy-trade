...
class InitialLetterGuarantee(models.Model):
    _name = 'initial.letter.guarantee'
    ...
    amount = fields.Monetary(currency_field='currency_id', string='Amount',readonly=True,default=0.00, required=True, store=True)
    bank_amount = fields.Monetary(currency_field='currency_id', string='Bank Expense', store=True)
    ...
    ], string='Type', default='initial', readonly=True,
        track_visibility='onchange')
    notice = fields.Text('Notice')
    tax = fields.Float(string='Tax Amount', default=0.00)
    request_lg_id = fields.Many2one('initial.request.letter.guarantee', string='Source Ref', store=True, readonly=True)
    ...
    def button_inprogress(self):
        if not self.journal_id.final_lg_account_id or not self.journal_id.lg_credit_account_id or not self.journal_id.bank_credit_account_id \
                or not self.journal_id.lg_account_id or not self.journal_id.tax_credit_account_id:
        ...
        lines = [
            (0, 0, {
                'name': 'خطاب الضمان الابتدائي',
                'account_id': self.journal_id.lg_credit_account_id.id,
                'credit': 0,
            ...
            (0, 0, {
                ...
                'account_id': self.journal_id.bank_credit_account_id.id,
                'credit': 0,
                'analytic_account_id': self.analytic_id.id,
            ...
            (0, 0, {
                ...
                'credit': self.tax + (self.amount * (self.journal_id.cover_percentage / 100)) + self.bank_amount,
                'debit': 0,
                'partner_id': self.partner_id.id,
                'date_maturity': self.date_issue,
            }),
    ...
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('initial.letter.guarantee') or 'New'
