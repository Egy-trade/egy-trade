...
class FinalRequestLetterGuarantee(models.Model):
    _name = 'final.request.letter.guarantee'
    ...
    state = fields.Selection([
        ('draft', 'Draft'),
        ('approved', 'PMO Approval'),
        ('financial_Approve', 'Reviewed'),
        ('reject', 'Rejected'),
        ('done', 'Done'),
    ...
    def get_final_lg(self):
        return {
            'name': 'Letter Of Guarantee',
            # 'view_type': 'form',
            'domain': [('request_lg_id', '=', self.id)],
            'view_mode': 'tree,form',
            'res_model': 'final.letter.guarantee',
    ...
    def action_create_lg(self):
        form_id = self.env.ref('letter_of_guarantee.final_wizard_letter_guarantee_view')
        return {
            # 'view_type': 'form',
            'view_mode': 'form',
    ...
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('final.request.letter.guarantee') or 'New'
